...
- [Decision](#decision)
  ...
  - [Fallthrough Decisions](#fallthrough-decisions)
    ...
    - [Example Walkthrough Program](#example-walkthrough-program-3)
...
Python uses indentation to indicate the scope of the nested `if` statements. There may or may not be code surrounding the nested `if` statements. Below is a flushed out example.

```python
# global scoped code block
if x:
    # "if x" code block (belongs to outer "if")
    if y:
        # "if y" code block (belongs to inner "if")
